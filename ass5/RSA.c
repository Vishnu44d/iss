...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/01 09:55:30 by Vishnu            #+#    #+#             */
/*   Updated: 2020/06/01 17:55:32 by Vishnu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    int i, bytes, len;
    i128 *p = i128_init(), *q = i128_init(), *n = i128_init();
    i128 *phi = i128_init(), *e = i128_init(), *d = i128_init();
    i128 *bbytes = i128_init(), *shift = i128_init();
    ...
    subtraction(temp1, p, &NUMS[1]);   // temp1 = p - 1
    subtraction(temp2, q, &NUMS[1]);   // temp2 = q - 1
    ...
    printf("Chosen public exponent, e = ");
    print_i128(e);
    printf("\nPublic key is: (");
    print_i128(e);
    printf(", ");
    ...
    {
        inplace_multiplication(bbytes, shift); // Shift by one byte
        bytes++;
    }
    ...
    if (f == NULL)
    {
        printf("Failed to open file \"message.txt\". Does it exist?\n");
        return EXIT_FAILURE;
    }
    ...
    //free all the resources. VERY IMPORTANT
    free(encoded);
    free(decoded);
    free(buffer);
    i128_free(p);
