...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/06/03 00:46:04 by Vishnu            #+#    #+#             */
/*   Updated: 2020/06/03 00:51:33 by Vishnu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
i128 *i128_init()
{
    i128 *b = (i128 *)malloc(sizeof(i128));
    b->length = 0;
    b->capacity = i128_CAPACITY;
    b->data = (unsigned int *)calloc(i128_CAPACITY, sizeof(unsigned int));
    return b;
}
...
void cpy(i128 *source, i128 *dest)
{
    ...
    if (source->capacity > dest->capacity)
    {
        ...
        dest->data = (unsigned int *)realloc(dest->data, dest->capacity * sizeof(unsigned int));
    }
...
void from_int_to_i128(i128 *b, unsigned int num)
{
    ...
    {
        b->capacity = b->length;
        b->data = (unsigned int *)realloc(b->data, b->capacity * sizeof(unsigned int));
    }
    b->data[0] = num;
}
...
void addition(i128 *result, i128 *b1, i128 *b2)
{
    ...
    {
        result->capacity = n + 1;
        result->data = (unsigned int *)realloc(result->data, result->capacity * sizeof(unsigned int));
    }
    for (i = 0; i < n; i++)
    {
        sum = carry;
...
void division(i128 *quotient, i128 *remainder, i128 *b1, i128 *b2)
{
    ...
    { /* Division by a single limb means we can do simple division */
        if (quotient->capacity < b1->length)
        {
            ...
            quotient->data = (unsigned int *)realloc(quotient->data, quotient->capacity * sizeof(unsigned int));
        }
        for (i = b1->length - 1; i >= 0; i--)
        {
    ...
    else
    {
        ...
        if (quotient->capacity < n - m)
        {
            ...
            quotient->data = (unsigned int *)realloc(quotient->data, (n - m) * sizeof(unsigned int));
        }
        cpy(b1, b1copy);
        cpy(b2, b2copy);
        ...
        if (b1copy->length != n)
        {
            ...
            if (b1copy->length > b1copy->capacity)
            {
                ...
                b1copy->data = (unsigned int *)realloc(b1copy->data, b1copy->capacity * sizeof(unsigned int));
            }
            b1copy->data[n - 1] = 0;
        }
        ...
        for (i = n - m - 1; i >= 0; i--)
        {
            ...
            {
                temp3->capacity = m + 1;
                temp3->data = (unsigned int *)realloc(temp3->data, temp3->capacity * sizeof(unsigned int));
            }
            temp3->length = 0;
            for (j = 0; j <= m; j++)
            {
...
{
    char *string = (char *)malloc((numDigits + 1) * sizeof(char));
    int i;
    string[0] = (rand() % 9) + '1';
...
int readFile(FILE *fd, char **buffer, int bytes)
{
    ...
    *buffer = (char *)malloc(BUF_SIZE * sizeof(char));
    ...
    if (len + bytes - len % bytes > cap)
        *buffer = (char *)realloc(*buffer, len + bytes - len % bytes);
    do
    {
        (*buffer)[len] = '\0';
        len++;
...
i128 *encodeMessage(int len, int bytes, char *message, i128 *exponent, i128 *modulus)
{
    ...
    int i, j;
    i128 *encoded = (i128 *)calloc(len / bytes, sizeof(i128));
